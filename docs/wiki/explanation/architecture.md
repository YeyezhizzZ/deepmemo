...
DeepMemo 是一个前后端分离的本地知识工作台。前端负责 Editor、QA、Wiki 三种工作模式，后端用 FastAPI 暴露文件、聊天、Wiki、引用和脉搏等接口；长期知识内容保存在本地 Markdown 文件里。
...
graph TB
    subgraph Frontend["Frontend: React + Vite"]
        ...
        Wiki["Wiki Mode<br/>Graph + Pages"]
    end

    subgraph Backend["Backend: FastAPI"]
        FS["FS Router<br/>data/ CRUD"]
        Chat["Chat Router<br/>SSE stream"]
        WikiR["Wiki Router<br/>graph + pages"]
    ...
    subgraph Storage["Storage"]
        ...
        WikiFiles["data/wiki<br/>generated Markdown"]
    end
...
- `src/routers/` 放用户可见 API：文件系统、聊天、Wiki、引用、脉搏等。
- `src/ai/` 放问答流水线：改写、路由、本地检索、Web fallback、答案合成。
- `src/wiki/` 放 Wiki ingest、图谱、健康报告、frontmatter 和存储逻辑。
...
用户提问时，聊天接口保存用户消息，构造最近 20 条上下文和较早摘要，调用 RAG 服务生成回答，再保存 AI 消息与 citations。Wiki 重建则从 diary 等源文件进入两阶段 LLM ingest，生成 `data/wiki/` 页面并更新图谱。
