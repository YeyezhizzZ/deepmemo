...
* **状态打标**：对发生变更的文件，在 `data.db` (`file_meta` 表) 中将其 `sync_status` 标记为 `dirty`，供其他组件（如 Wiki 系统）消费。

## 3. Evidence
* `src/app/core/watcher.py`

## 4. Current Flow
启动 FastAPI 时挂载 Watchdog 线程 -> `data/` 目录发生系统级变动 -> 过滤非 Markdown 文件 -> 计算 MD5 -> 与 DB 对比 -> 更新 `sync_status` 为 dirty。
...
## 7. Regression Risks
* 修改文件元数据状态可能导致前后端同步状态不一致，或导致 Wiki 系统无法正确触发构建。
