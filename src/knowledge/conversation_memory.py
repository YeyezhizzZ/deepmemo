...
class ConversationMemoryExtractor:
    ...
    def extract(self, session_id: str, messages: list[dict]) -> list[KnowledgeCard]:
        ...
        if signal_type is None:
            return []
        raw_path = self._write_conversation_raw(session_id, messages)
        card = self._generate_card_draft(signal_type, messages, raw_path)
    ...
    def _generate_card_draft(self, signal_type: str, messages: list[dict], raw_path: str) -> KnowledgeCard:
        user_lines = [
        ...
        return KnowledgeCard(
            ...
            sources=[EvidenceSource(path=raw_path, evidence=definition, confidence=0.75)],
            tags=["conversation", signal_type],
        )

    def _write_conversation_raw(self, session_id: str, messages: list[dict]) -> str:
